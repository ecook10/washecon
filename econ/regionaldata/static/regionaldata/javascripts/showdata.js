...
$(document).ready(function() {
    ...
    //var chart_url = "/regionaldata/getdata?";
    var chart_url = 'http://bea.gov/api/data/?';

    $('input').each(function() {
        chart_url += '&' + $(this).attr('id') + '=' + $(this).val();
    });

    $.getJSON(chart_url, function(data) {

        if (data.BEAAPI.Results.hasOwnProperty('Error')) {      //Not extendable
            $('#chart, #options').css('display', 'none');
            $('#error div').text(data.BEAAPI.Results.Error.APIErrorDescription);
            $('#error').css('display', '');
            ...
            var title_array = data.BEAAPI.Results.PublicTable.split(' ');
            var title = title_array.splice(1, title_array.length).join(' ');
    ...
    var compare_url = "http://bea.gov/api/data/?UserID=" + user_key
        + "&method=GetParameterValues"
        + "&datasetname=RegionalData"
        + "&ParameterName=KeyCode";

    $.getJSON(compare_url, function(data) {
...
function getDataPoints(data, x_axis_key, y_axis_key) {
    ...
    for (var i = 0; i < data.length; i++) {
        //console.log(data[i][x_axis_key]);
        final.push([Date.UTC(data[i][x_axis_key], 0), parseInt(data[i][y_axis_key])]);
    }
    return final;
}
